...
Every Fleet and combat movement step is checked against the same embedded coastline rendered by the operating map. Mission execution, formation movement, retreat, engagement intercepts, commercial escape, and Blackwake pursuit use swept-path validation with a shoreline margin; an offshore endpoint cannot create a shortcut across an island. The navigator deterministically steers around blocked shoreline segments, and legacy persisted positions on land are reconciled to the nearest safe-water point before movement resumes.
