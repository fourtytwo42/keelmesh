...
Coastline safety coverage forces both controlled-vessel and Blackwake movement toward an endpoint across Block Island. Every swept step remains in water, the navigator reaches the opposite side by routing around the rendered land polygon, and persisted land positions reconcile to safe water. The full Go suite and vet pass with the exact map fixture embedded in the runtime. A deployed 500× rehearsal advanced the live simulation by more than seven world hours and rechecked all 45 visible entities against the rendered land polygons with zero incursions; the operator rate was then restored to 20×.
