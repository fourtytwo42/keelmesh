...
func validateWorkspaceCommand(value domain.WorkspaceAssistantResponseV1, request domain.WorkspaceAssistantRequestV1, fleet domain.FleetSnapshotV2) error {
	...
	allowed := map[string]bool{"open_window": true, "close_window": true, "select_group": true, "select_vessel": true, "select_all": true, "clear_selection": true, "inspect_group": true, "inspect_vessel": true, "inspect_contact": true, "set_simulation_rate": true, "set_theme": true, "create_mission": true, "choose_plan": true, "open_mission": true, "pause_mission": true, "resume_mission": true, "delete_mission": true, "create_group": true, "delete_group": true, "move_vessel_to_group": true, "plan_engagement": true, "repair_vessel": true, "arm_vessel": true, "disarm_vessel": true, "enable_auto_defense": true, "disable_auto_defense": true, "none": true}
	...
	for _, action := range value.Actions {
		...
		case "arm_vessel", "disarm_vessel", "enable_auto_defense", "disable_auto_defense":
			ids := action.TargetIDs
			if len(ids) == 0 {
				ids = []string{action.Target}
			}
			if len(ids) == 0 {
				return errors.New("arm state target required")
			}
			for _, id := range ids {
				if !vessels[strings.ToLower(strings.TrimSpace(id))] {
					return errors.New("unknown arm state target")
				}
			}
		case "plan_engagement":
			...
			for _, entity := range fleet.Combat.Entities {
				if strings.EqualFold(entity.EntityID, action.Target) || strings.EqualFold(entity.Name, action.Target) || strings.EqualFold(entity.BoatID, action.Target) {
					eligible = !entity.Profile.Controlled
				}
			}
			if !eligible || len(action.TargetIDs) == 0 {
				return errors.New("non-fleet target and controlled participants required")
			}
...
func deterministicWorkspaceCommand(request domain.WorkspaceAssistantRequestV1, fleet domain.FleetSnapshotV2) domain.WorkspaceAssistantResponseV1 {
	...
	armIntent := strings.Contains(lower, "arm ") || strings.HasPrefix(lower, "arm") || strings.Contains(lower, "weapons ready")
	disarmIntent := strings.Contains(lower, "disarm") || strings.Contains(lower, "weapons safe")
	if (armIntent || disarmIntent) && len(mentionedVessels) > 0 {
		result.Mode = "workspace"
		kind, state := "arm_vessel", "armed for bounded return fire"
		if disarmIntent {
			kind, state = "disarm_vessel", "weapons safe"
		}
		result.Speech = fmt.Sprintf("I set %d vessel%s %s. Arming alone does not authorize pursuit or initiating an attack.", len(mentionedVessels), map[bool]string{true: "", false: "s"}[len(mentionedVessels) == 1], state)
		result.Actions = []domain.WorkspaceAssistantActionV1{{Kind: kind, Target: mentionedVessels[0], TargetIDs: mentionedVessels}}
		return result
	}
	autoDefenseIntent := strings.Contains(lower, "auto defense") || strings.Contains(lower, "automatic defense") || strings.Contains(lower, "auto defence") || strings.Contains(lower, "automatic defence")
	if autoDefenseIntent && len(mentionedVessels) > 0 {
		disable := strings.Contains(lower, "disable") || strings.Contains(lower, "turn off") || strings.Contains(lower, "switch off")
		response := "retreat"
		if strings.Contains(lower, "retaliat") || strings.Contains(lower, "return fire") {
			response = "retaliate"
		}
		kind, state := "enable_auto_defense", "enabled with retreat priority"
		if disable {
			kind, state = "disable_auto_defense", "disabled; attacks will notify while vessels maintain their assigned state"
		} else if response == "retaliate" {
			state = "enabled for retaliation when armed"
		}
		result.Mode = "workspace"
		result.Speech = fmt.Sprintf("Automatic defense is %s for %d vessel%s.", state, len(mentionedVessels), map[bool]string{true: "", false: "s"}[len(mentionedVessels) == 1])
		result.Actions = []domain.WorkspaceAssistantActionV1{{Kind: kind, Target: mentionedVessels[0], TargetIDs: mentionedVessels, SecondaryTarget: response}}
		return result
	}
	...
	if engagementIntent {
		targetID := mentionedContactID
		if targetID == "" {
			for _, entity := range fleet.Combat.Entities {
				if entity.Profile.Controlled {
					continue
				}
				if strings.Contains(lower, strings.ToLower(entity.Name)) || strings.Contains(lower, strings.ToLower(entity.EntityID)) || (entity.Profile.Hostility == "hostile" && (strings.Contains(lower, "pirate ship") || strings.Contains(lower, "hostile"))) {
		...
		if targetID != "" && len(participants) > 0 {
			result.Mode = "workspace"
			result.Speech = "I prepared a bounded fictional engagement with the resolved participants and contact. Review and confirm the exact engagement hash before any vessel may initiate fire."
			result.Actions = []domain.WorkspaceAssistantActionV1{{Kind: "plan_engagement", Target: targetID, TargetIDs: participants}}
			return result
		}
		result.Speech = "Which controlled vessel or group should engage which contact?"
		return result
	}
...
func fixtureIncident(commit string) domain.IncidentManifestV1 {
	...
	evidence := []domain.IncidentEvidenceV1{{ID: "ev-link-relay", Kind: "link", SourceID: "m2-audit-41", Summary: "Vessel 4 lost direct Starlink and selected Vessel 3 HaLow relay.", Trust: "authenticated", Tick: 20}, {ID: "ev-partition", Kind: "link", SourceID: "m2-audit-57", Summary: "Vessel 4 became fully partitioned and continued beyond one minute from its complete signed program.", Trust: "authenticated", Tick: 75}, {ID: "ev-spoof", Kind: "pnt", SourceID: "m2-pnt-650m", Summary: "GNSS jumped 650 m northeast with inconsistent velocity and clock evidence; fix was excluded.", Trust: "sensor_fused", Tick: 90}, {ID: "ev-hold", Kind: "contingency", SourceID: "m2-audit-89", Summary: "Uncertainty exceeded 45 m and triggered the signed safe-hold contingency.", Trust: "policy", Tick: 90}, {ID: "ev-bridge", Kind: "rejoin", SourceID: "m2-audit-113", Summary: "Restored HaLow reconciled program revision, watermark, decision epoch, and fused pose without stale movement replay.", Trust: "policy", Tick: 105}}
