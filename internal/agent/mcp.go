...
func (m *Manager) MCPHandler() http.Handler {
	...
	toolNames := []string{"incident.get_manifest", "telemetry.get_window", "audit.get_timeline", "pnt.get_evidence", "execution.get_program_lifecycle", "policy.explain_decision", "runbook.search", "history.find_similar", "simulation.replay_incident", "evaluation.draft_candidate"}
...
func (m *Manager) callTool(_ context.Context, name string, raw json.RawMessage) (*mcp.CallToolResult, error) {
	...
	case "telemetry.get_window":
		value = map[string]any{"incident_id": incident.ID, "window": map[string]int64{"start_tick": 35, "end_tick": 95}, "records": []map[string]any{{"tick": 35, "link": "partitioned", "complete_program_onboard": true}, {"tick": 75, "continued_beyond_one_minute": true, "decision_scope": "local"}, {"tick": 90, "gnss_offset_m": 650, "fused_position_changed": false}, {"tick": 95, "uncertainty_m": 48, "behavior": "safe_hold"}}, "bounded": true}
	...
	case "execution.get_program_lifecycle":
		value = map[string]any{"complete_program_onboard": true, "program_duration_seconds": 7200, "authorization_expiry_tick": 7200, "continued_beyond_one_minute": true, "decision_scope": "local", "stale_movement_replayed": false, "execution_watermark": 7}
	case "policy.explain_decision":
		value = map[string]any{"decision": "safe_hold_then_bridge", "reasons": []string{"PNT_UNSAFE", "SIGNED_CONTINGENCY", "NO_STALE_REPLAY", "FUTURE_LEAD_TIME_REQUIRED"}, "mission_authority_mutated": false}
	case "runbook.search":
...
func runbookChunks(query string) []map[string]any {
	chunks := []map[string]any{{"source_id": "runbook-gnss-anomaly-v2", "chunk_id": "runbook-gnss-anomaly-v2#integrity", "title": "GNSS anomaly response", "trust": "approved", "excerpt": "Reject position fixes that disagree with inertial, radar, peer-relative, velocity, or clock evidence. Increase uncertainty and constrain motion."}, {"source_id": "runbook-comms-loss-v4", "chunk_id": "runbook-comms-loss-v4#program", "title": "Communications loss and full-program execution", "trust": "approved", "excerpt": "Continue the complete signed and unexpired onboard program within its authority envelope. At mission expiry or an unsafe guardrail, invent no new work and enter the signed contingency."}, {"source_id": "runbook-stale-rejoin-v1", "chunk_id": "runbook-stale-rejoin-v1#bridge", "title": "Stale-safe reconnection", "trust": "approved", "excerpt": "Exchange program revision, execution watermark, decision epoch, and fused pose, then bridge to current work without replaying missed movement."}}
