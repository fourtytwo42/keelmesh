...
container. Use `scripts/keelmesh start` for routine starts and rebuilds. The
wrapper detects the private referee runtime and automatically includes
`compose.m12.yaml`; do not recreate VM 214 core with bare `docker compose` or it
will intentionally fall back to the base simulated gateway. The verified VM 214
deployment and all twelve voters have used authoritative `raft` mode since
2026-09-04. The `simulated` and `shadow` modes remain rollback and comparison
paths for explicitly configured environments.
