...
export function FleetWorkspace() {
  ...
  async function createMission() {
    ...
    await createMissionFor([...selected].filter((id) => !occupied.has(id)));
  }
  ...
  async function setVesselArmed(vesselID: string, armed: boolean) {
    try {
      await api<CombatEntityStateV1>(`/api/v8/combat/vessels/${vesselID}:${armed ? "arm" : "disarm"}`, {
        method: "POST",
        body: JSON.stringify({
          request_id: requestID("combat-arm"),
          idempotency_key: requestID("combat-arm-key"),
          expected_version: fleet?.combat.state_version ?? 0,
          actor_identity: "demo-operator",
          armed,
        }),
  ...
  async function createPlans(
	...
	strategyCount: 1 | 3 = 3,
	showPlanner = true,
    engagementPolicy?: EngagementPolicyV1,
  ) {
  ...
  async function applyAssistantAction(action: WorkspaceAssistantActionV1) {
	...
	if (action.kind === "arm_vessel" || action.kind === "disarm_vessel") {
	  const vesselIDs = resolveActionVesselIDs(action);
  ...
  async function generateManualMission(missionType: string, objective: string, engagementPolicy?: EngagementPolicyV1) {
    if (!mission) return;
    ...
    await createPlans(current, intent, "manual", missionType, "", true, 1, true, engagementPolicy);
  }

  async function refineMissionWithAI(missionType: string, objective: string, instruction: string, alternatives: boolean, engagementPolicy?: EngagementPolicyV1) {
    if (!mission) return;
    const current = await missionWithObjective(mission, objective);
    if (!current) return;
    ...
    await createPlans(current, intent, "ai_assisted", missionType, "", false, alternatives ? 3 : 1, true, engagementPolicy);
  }
  ...
  async function performGuidedDemoAction(action: GuidedDemoAction) {
    ...
    if (action === "create-ai-mission") {
      ...
      // A bounded loop keeps the complete-program authority visible throughout
      // the natural-cadence 100x narration without requiring an unsafe route.
      const intent = "Create one mission for Harbor Sentinel to patrol two nautical miles east and loop continuously until authorization expires, preserve at least thirty percent battery, and maintain safe depth and separation. Do not offer alternatives.";
    ...
    defs.push({
      ...
      content: (
        <MissionCanvas
          ...
          onTool={setTool}
          onGenerateManual={(missionType, objective, policy) => void generateManualMission(missionType, objective, policy)}
          onRefineAI={(missionType, objective, instruction, alternatives, policy) => void refineMissionWithAI(missionType, objective, instruction, alternatives, policy)}
    ...
    <main className={`m6-shell ${demoState.running ? "demo-running" : ""} demo-focus-${demoState.focus}`}>
      <header className="ops-bar">
        ...
        <nav>
          ...
          <button aria-label={words.mission} title="Create a new mission" onClick={createMission}>
            <Route />
      ...
      {pendingEngagement && (
        <div className="mission-delete-backdrop">
          <section className="mission-delete-dialog combat-approval-dialog" role="dialog" aria-modal="true" aria-labelledby="combat-approval-title">
            ...
            <p>{pendingEngagement.participant_ids.length} controlled vessel(s) may engage {fleet.surface_contacts.find((contact) => contact.id === pendingEngagement.target_id)?.name ?? pendingEngagement.target_id} for up to {Math.ceil(pendingEngagement.duration_seconds / 60)} world minutes, inside a {pendingEngagement.maximum_range_m.toFixed(0)} m weapon envelope and {pendingEngagement.maximum_effects} maximum effects.</p>
...
function MissionCanvas({ pirate, mission, groups, plans, activePlan, busy, tool, contactSeed, geometryFocus, showExecution, onFormation, onLoop, onSaveDraft, onArea, onTool, onGenerateManual, onRefineAI, onOpenConstraints, onApplyContactSeed, onClearContactSeed, onUndoGeometry, onClearGeometry, onDeleteGeometry, onFocusGeometry, onReorderWaypoint, onChoose, onConfirmPlan, onStatus, onRename, onDelete }: {
  ...
  onGenerateManual: (missionType: string, objective: string, engagementPolicy?: EngagementPolicyV1) => void;
  onRefineAI: (missionType: string, objective: string, instruction: string, alternatives: boolean, engagementPolicy?: EngagementPolicyV1) => void;
  ...
  return <div className="planner mission-editor mission-command-center">
    ...
    <div className={`mission-scope-line ${mission.target_ids.length ? "ready" : "empty"}`}><Users /><span><b>{mission.target_ids.length ? scopeParts.join(" · ") : "No assets selected"}</b><small>{mission.target_ids.length ? `${mission.target_ids.length} Fleet selection${mission.target_ids.length === 1 ? "" : "s"} assigned to this mission` : "Select vessels or groups in Fleet; this mission updates automatically."}</small></span></div>
    <nav className="mission-workflow-tabs" aria-label="Mission workflow">
      <button className={workspaceTab === "plan" ? "active" : ""} onClick={() => setWorkspaceTab("plan")}><span>1</span><Compass /><b>Plan</b><small>Task, route &amp; guardrails</small></button>
      <button className={workspaceTab === "route" ? "active" : ""} onClick={() => setWorkspaceTab("route")}><span>2</span><ShieldCheck /><b>Review &amp; Run</b><small>Validate, authorize &amp; execute</small></button>
    </nav>
