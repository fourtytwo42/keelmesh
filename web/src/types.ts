...
export type VesselClassV2 = { id:string; name:string; role:string; max_speed_mps:number; minimum_reserve:number; endurance_hours:number; communications_role:boolean };
export type EnvironmentV2 = { wind_speed_mps:number; wind_direction_deg:number; current_speed_mps:number; current_direction_deg:number; wave_height_m:number; water_temperature_c:number; fixture_at:string; label:string; source_ids:string[] };
export type VesselProfileV2 = { schema_version:number; id:string; designation:string; callsign:string; display_name:string; class:VesselClassV2; group_id:string; group_code:string; group_color:string; group_pattern:string; available:boolean; telemetry:{ position:Point; heading_deg:number; speed_mps:number; reserve:number; projected_reserve:number; mode:string; health:string; pnt_integrity:string; uncertainty_m:number; tape_depth_seconds:number; mission_id?:string; route:Point[]; environment:EnvironmentV2 } };
export type OperationalGroupV2 = { schema_version:number; id:string; code:string; name:string; color:string; pattern:string; member_ids:string[]; revision:number };
export type SavedCollectionV2 = { schema_version:number; id:string; name:string; member_ids:string[]; revision:number };
export type ReachabilityPathV2 = { vessel_id:string; state:string; hops:string[]; underlay:string[]; latency_ms:number };
export type ReachabilityV2 = { schema_version:number; vessel_id:string; authority:string; direct_peers:ReachabilityPathV2[]; relayed_peers:ReachabilityPathV2[]; unreachable_group_members:string[]; reachable_outside_group:ReachabilityPathV2[] };
export type ConstraintSetV2 = { minimum_reserve:number; maximum_speed_mps:number; minimum_vessel_separation_m:number; minimum_object_separation_m:number; maximum_wave_height_m:number; maximum_wind_mps:number; maximum_pnt_uncertainty_m:number; maximum_duration_minutes:number; maximum_route_distance_km:number; maximum_shore_distance_m?:number; minimum_tape_watermark_seconds:number; formation:string; formation_spacing_m:number; leader_policy:string; regroup_threshold_m:number };
export type MissionPOIV2 = { id:string; name:string; kind:string; position:Point; radius_m:number };
export type MissionWaypointV2 = { id:string; position:Point; color:"amber"|"red"|"green"|"cyan"|"violet"|"white"; sequence:number };
export type MissionGeometryV2 = { revision:number; included_areas:number[][][]; exclusion_areas:number[][][]; waypoints:Point[]; waypoint_details?:MissionWaypointV2[]; pois:MissionPOIV2[] };
export type MissionWorkspaceV2 = { schema_version:number; id:string; name:string; objective:string; status:string; target_ids:string[]; target_snapshot_hash:string; fleet_version:number; version:number; geometry:MissionGeometryV2; constraints:ConstraintSetV2; formation:string; plan_ids:string[]; authorized_plan_id?:string; created_at:string; updated_at:string };
export type MissionStrategyV2 = { id:string; name:string; description:string; formation:string; guidance_kind:string; speed_factor:number; reserve_bias:number; maneuvers:string[] };
export type MissionAdvisorV2 = { state:string; provider:string; model:string; summary:string; strategies:MissionStrategyV2[]; attempts:ProviderAttempt[] };
export type CommandDraftV2 = { schema_version:number; id:string; mission_id:string; source_text:string; objective:string; target_ids:string[]; target_snapshot_hash:string; geometry_revision:number; fleet_version:number; constraints:ConstraintSetV2; formation_preference:string; guidance_kind:string; waypoints:Point[]; geometry_source?:string; resolution_notes?:string[]; unresolved_ambiguities:string[]; advisor:MissionAdvisorV2; content_hash:string };
export type FleetAssignmentV2 = { vessel_id:string; route:Point[]; speed_mps:number; distance_km:number };
export type FleetPlanV2 = { schema_version:number; id:string; mission_id:string; draft_id:string; name:string; description:string; formation:string; advisor_source:string; advisor_model?:string; maneuvers:string[]; assignments:FleetAssignmentV2[]; coverage_percent:number; minimum_reserve:number; duration_minutes:number; energy_kwh:number; link_exposure_seconds:number; minimum_separation_m:number; policy_status:string; reason_codes:string[]; recommended:boolean; content_hash:string; source_mission_version:number };
export type FleetPreviewV2 = { plan_id:string; plan_hash:string; duration_seconds:number; routes:Record<string,Point[]>; nothing_sent:boolean };
export type FleetLeaseV2 = { id:string; mission_id:string; plan_id:string; plan_hash:string; operator_id:string; target_ids:string[]; issued_at:string; expires_at:string; signature:string };
export type FleetSnapshotV2 = { schema_version:number; fleet_version:number; generated_at:string; vessels:VesselProfileV2[]; groups:OperationalGroupV2[]; collections:SavedCollectionV2[]; missions:MissionWorkspaceV2[]; environment:EnvironmentV2; map:{name:string;center:Point;bounds:Point[];fixture:boolean;navigation_warning:string} };
export type VoiceV2 = { id:string; name:string; default:boolean; available:boolean };
export type ArenaNodeV1={id:string;faction:"A"|"B";vessel_id:string;planned_vm_id:number;planned_management_ip:string;host:string;role:string;status:string;radio_state:string;management_connected:boolean;inference_connected:boolean;provider:string;position:Point;heading_deg:number;speed_mps:number;battery_kwh:number;battery_capacity_kwh:number;solar_kw:number;hull:number;hull_maximum:number;class:string;equipment:string[];tape_depth_seconds:number};
export type CoordinatorV1={faction:string;node_id:string;epoch:number;votes:number;quorum_required:number;state:string;failover_count:number;recovery_ms:number};
export type ContactTrackV1={id:string;faction:string;classification:string;hostility:string;position:Point;heading_deg:number;speed_mps:number;confidence:number;uncertainty_m:number;source:string;last_seen_tick:number;state:string};
export type WorkspaceActionV1={id:string;session_id:string;faction:string;kind:string;authority_class:string;arguments:Record<string,unknown>;state:string;result_hash:string;at:string};
export type AgentSessionV1={id:string;faction:string;state:string;coordinator_id:string;coordinator_epoch:number;message:string;actions:WorkspaceActionV1[];awaiting_approval:boolean};
export type EngagementPlanV1={id:string;faction:string;coordinator_epoch:number;friendly_node_ids:string[];target_track_ids:string[];equipment:string[];maximum_effects:number;starts_tick:number;expires_tick:number;summary:string;content_hash:string;policy_status:string};
export type EngagementLeaseV1={id:string;plan_id:string;plan_hash:string;faction:string;coordinator_epoch:number;operator_id:string;remaining_effects:number;expires_tick:number;signature:string};
export type ArenaSnapshotV1={schema_version:number;state_version:number;match_id:string;mode:string;phase:string;simulation_rate:number;mission_tick:number;simulated_time:string;viewer_faction:string;credits:Record<string,number>;nodes:ArenaNodeV1[];coordinators:CoordinatorV1[];knowledge:{faction:string;friendly_ids:string[];contacts:ContactTrackV1[];checksum:string};workspace_actions:WorkspaceActionV1[];engagement_plan?:EngagementPlanV1;engagement_lease?:EngagementLeaseV1;events:{sequence:number;tick:number;kind:string;faction?:string;summary:string}[];management_plane:string;inference_plane:string;radio_plane:string;provisioning_state:string;provisioning_blockers:string[]};
