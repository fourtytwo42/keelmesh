...
import { Expand, Minus, PanelLeft, PanelRight, Shrink, SquareDashed, Trash2, X } from "lucide-react";
  ...
  return <>
    {windows.map(w=>{const s=states[w.id]??defaults[w.id];if(!s||s.closed||s.minimized)return null;const top=clamp(s.y,82,Math.max(82,window.innerHeight-reservedBottom-180));const style=s.maximized?{left:8,top:82,width:window.innerWidth-16,height:window.innerHeight-82-reservedBottom,zIndex:s.z}:s.dock?{left:s.dock==="left"?0:undefined,right:s.dock==="right"?0:undefined,top:82,width:Math.min(390,window.innerWidth*.34),height:window.innerHeight-82-reservedBottom,zIndex:s.z}:{left:s.x,top,width:s.width,height:Math.min(s.height,window.innerHeight-top-reservedBottom),zIndex:s.z};return <section key={w.id} className={`float-window ${s.dock?`docked ${s.dock}`:""} ${s.maximized?"maximized":""}`} style={style} onPointerDown={()=>focus(w.id)} onKeyDown={e=>{if(!(e.altKey&&["ArrowLeft","ArrowRight","ArrowUp","ArrowDown"].includes(e.key)))return;e.preventDefault();const step=e.shiftKey?40:10;mutate(w.id,{dock:undefined,maximized:false,x:clamp(s.x+(e.key==="ArrowLeft"?-step:e.key==="ArrowRight"?step:0),0,window.innerWidth-180),y:clamp(s.y+(e.key==="ArrowUp"?-step:e.key==="ArrowDown"?step:0),82,window.innerHeight-reservedBottom-180)})}} tabIndex={0} aria-label={w.title}>
      <header className="float-title" onDoubleClick={()=>mutate(w.id,{dock:s.dock?undefined:(w.preferredDock??"right")})} onPointerDown={e=>{if((e.target as HTMLElement).closest("button"))return;drag.current={id:w.id,mode:"move",startX:e.clientX,startY:e.clientY,initial:s};(e.currentTarget as HTMLElement).setPointerCapture?.(e.pointerId)}}>
        ...
        <button aria-label={`Snap ${w.preferredDock??"left"}`} title={`Snap ${w.preferredDock??"left"}`} onClick={()=>{const side=w.preferredDock??"left";mutate(w.id,{dock:s.dock===side?undefined:side,maximized:false})}}>{(w.preferredDock??"left")==="right"?<PanelRight/>:<PanelLeft/>}</button>
