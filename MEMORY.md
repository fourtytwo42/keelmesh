...
- Current task: the workspace density, hover-help, technical-window, and navigation-toggle pass is implemented, verified, and deployed.
- Last meaningful change: Mission Planner opens without scrolling, can shrink to a chat-only surface, and uses collapsed authoring/objective sections for progressive disclosure. Fleet can shrink to 245 px. Engineer and Cutaway use the graphite/amber workspace visual system without default overflow. The top bar contains only Fleet, Mission, Engineer, and Cutaway; each button and each active mission tab toggles its window.
...
- Mission Planner layout, last verified 2026-09-03: floating defaults to a compact no-scroll workspace, optional right docking remains readable at 390 px, and maximized mode uses a balanced two-pane layout. At its 350×245 minimum the Planner intentionally becomes a chat-and-voice-only surface; Assigned Assets stays visible in the normal default while Objective and Map Authoring use one-click progressive disclosure. The maximized transcript is content-bounded instead of stretching into oversized empty cards.
