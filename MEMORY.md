...
- Current task: the operating fleet is now the twelve provisioned vessel VMs rather than a separate 48-vessel fixture.
- Last meaningful change: each Fleet vessel has one persistent VM/node binding, real management-health projection, simulated Starlink/HaLow and GNSS integrity state, an unassigned baseline, and a shoreline-validated dispersed open-water spawn.
- Next step: broader M7 physical mesh replication remains separate; do not describe simulated HaLow/GNSS fault state as physical radio or GPS hardware testing.
