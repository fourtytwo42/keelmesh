...
- Last meaningful change: VM 214's LAN and embedded-browser startup path is repaired after M11. The app no longer depends on secure-context-only `crypto.randomUUID`, HTML is never cached, the bundle targets ES2020, module-load failures produce a recovery surface, and MapLibre retains its full workspace dimensions after lazy CSS loads.
