...
- Current task: spoken operational-group colors and the VM-local Jarvis Pocket TTS voice are deployed on VM 214 and all twelve vessel nodes.
- Last meaningful change: the eight seeded groups now have stable human-readable color identities carried through fleet contracts, vessel state, waypoint menus, AI planning context, and map rendering. The newly cloned Jarvis Pocket TTS state is installed as a VM-local ignored runtime artifact and is the default voice; built-in voices remain available.
...
- Result: VM 214 and all twelve vessel nodes are healthy on core binary SHA-256 `f69d90d431a90b02b1ff9e6bca8d0470ad5887d0cb3d666421625d0a09f5724c`. The custom voice remains outside Git and container images. No GitHub-hosted workflow or Proxmox snapshot ran.
