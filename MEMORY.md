...
- Current task: complete the Fleet-driven Mission command-center redesign.
- Last meaningful change: Mission is now a compact two-stage Plan / Review & Run workspace. Fleet is the only asset-selection surface; its current vessel/group selection automatically updates the active mission. The top Mission navigation action creates a new mission exactly like `+`.
- Next step: commit/push the verified implementation without hosted workflows.
