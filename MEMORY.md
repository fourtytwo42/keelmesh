...
- Current task: the Mission Planner density redesign is implemented, verified, deployed, and pushed in commit `ac28d23`.
- Last meaningful change: floating, snapped-right, and maximized Mission Planner modes now share a compact hierarchy. Assigned Assets is a collapsible section with formation controls integrated into it; redundant pending-state cards were removed; chat, objective, authoring, route, and action areas have mode-specific density rules rather than uniformly stretching.
...
- Mission Planner layout, last verified 2026-09-03: floating defaults to a compact 450×640 workspace, optional right docking remains readable at 390 px, and maximized mode uses a balanced two-pane layout. Assigned assets, formation, objective, map authoring, chat, route candidates, and primary actions use consistent compact spacing. The maximized transcript is content-bounded instead of stretching into oversized empty cards.
