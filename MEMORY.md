...
- Current task: controlled-vessel follow/intercept speed calibration is implemented, verified, and deployed.
- Last meaningful change: fictional surface traffic now spans 1.2–2.8 m/s while Atlas, Mariner, and Kestrel hardware limits are 3.0, 3.2, and 3.4 m/s. Contact-follow compilation raises only that mission's exact-approval speed envelope to the slowest selected vessel, and every generated strategy retains positive closure speed without exceeding hardware limits.
...
- Fictional surface traffic, last verified 2026-09-03: the M6 operating picture includes twelve deterministic AIS-like contacts across container ship, tanker, ferry, trawler, fictional patrol vessel, and yacht classes. Each has a stable generated name, Boat ID, callsign, color identity, speed/course/dimensions/activity, and looped programmed route. NPC simulation speeds span 1.2–2.8 m/s; controlled Atlas/Mariner/Kestrel classes top out at 3.0/3.2/3.4 m/s so every class has positive intercept headroom. Contact-follow planning automatically uses an approval-bound overtake envelope and each strategy remains faster than its target while respecting the slowest selected vessel. Left-click opens a movable detail inspector. Mission intent may identify a contact by exact name, callsign, Boat ID, or unique color; contacts remain simulation-only and cannot be directly commanded. Their neutral generated art is unchanged by Pirate mode.
