...
- Current task: the former Cutaway and Engineer views are now distinct System and AI Lab workspaces, deployed to VM 214 and all twelve vessel nodes without GitHub-hosted workflows or snapshots.
- Last meaningful change: System presents live mission authority, all twelve node/management/radio states, the Kafka/PostgreSQL pipeline, recovery controls, event transitions, and measured scale evidence. AI Lab is reserved for provider, MCP, retrieval/citation, replay, evaluation, approval, and trace evidence, with direct cross-navigation between the views.
