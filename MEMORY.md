...
- Current task: the mission-centered map interaction refactor is implemented, verified, deployed, and pushed in commit `70630db`.
- Last meaningful change: all authoring tools now live inside the active Mission Planner; ordinary map context is read-only, contact planning begins as an uncommitted seed, and manual route generation visibly bypasses every model provider.
