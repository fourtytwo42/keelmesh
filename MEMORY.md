...
- Current task: the Mission window is now a manual-first mission editor with optional, explicitly invoked AI refinement; it is deployed to VM 214 and all twelve vessel nodes without GitHub-hosted workflows or snapshots.
- Last meaningful change: embedded mission chat, history, microphone, and AI scene surfaces were removed from Mission. Operators directly edit mission type, objective, assets, geometry, formation, constraints, and routes; deterministic Build Route bypasses providers, while a collapsed Refine with AI drawer can apply one refinement or offer three previewable alternatives before the unchanged exact-plan confirmation boundary.
