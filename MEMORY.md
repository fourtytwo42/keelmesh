...
- Last meaningful change: the primary workspace windows are labeled `Fleet` and `Mission` (Pirate: `Flotilla` and `Voyage`). Fleet group and vessel rows expose compact gold-border live hover summaries for identity, motion/formation, reserve, PNT, health, and decision-node state.
