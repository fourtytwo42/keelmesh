...
- Current task: implement and deploy M11 Distributed Agent Memory and Learning Plane without GitHub-hosted workflows or snapshots.
- Last meaningful change: central scoped memory, hybrid pgvector/full-text retrieval, context assembly, durable global assistant turns, Kafka memory topics/worker, node-local SQLite WAL+bbolt storage, private MCP memory tools, ONNX MiniLM embedding endpoint, and Cutaway memory telemetry are implemented on branch `m11-memory`; Linux build/deployment verification is next.
- Next step: apply the M11 patch to canonical VM 214, update `go.sum`, run Go/Python/TypeScript/Compose gates, deploy, exercise `/api/v5`, then distribute the verified node binary.
- Blockers: none currently. The optional `memory-lab` profile may require first-time image downloads and is not a mission-runtime dependency. No snapshot is authorized.
...
- M6 is live on VM 214. It serves a graphite MapLibre workspace with a packaged NOAA NCDS chart extract, 48 persistent named vessels in eight exclusive operational groups, overlapping saved collections, scalable symbol layers, fleet/group/search/map selection, vessel/environment/reachability inspection, movable/dockable windows, PostgreSQL-backed mission workspaces, deterministic formation candidates, and exact-plan authorization. Mission rectangle assignment, areas, exclusions, waypoints, hold/orbit points, geometry inventory, undo, and clearing are scoped to the visible active Mission Planner. Fleet/Groups optionally snaps left and Mission Planner optionally snaps right. Manual planning makes no provider request; AI assistance feeds the same deterministic planning and authority boundary. Pocket TTS exposes twelve built-in voices plus VM-local custom Jarvis and Captain Barbossa voices; Jarvis is the Navy default and Barbossa is the Pirate default. Pinned faster-distil-whisper-small.en int8 provides VM-local HTTP/WebSocket transcription with typed fallback. Browser WebGPU/WASM and trusted-peer speech routing are accurately labeled future tiers, not completed redundancy.
...
- M10 is live on VM 214 and all twelve vessel nodes. The assistant produces strict scene intents that trusted Go code resolves into ordered A2UI v1.0 surfaces using pinned `@a2ui/react` 0.11.0, React 19.2.8, and Zod 3.25.76. One unpinned scene is active per browser session; up to four pinned scenes survive reload, scene values refresh from current fleet state without a model call, scene annotations are independent from durable mission overlays, and separate sessions cannot replace/read/mutate one another. The private MCP exposes session-scoped scene list/get/compose tools but no authorization or effect execution. Conversation detail remains behind Assistant History; Mission Planner is now presented as Mission Canvas. Critical PNT, tape, and reserve transitions are detected deterministically and composed without granting model authority.
- M11 implementation in progress, last verified 2026-09-03: Go is the memory authority for scoped persistence/retrieval/promotion/tombstones/context receipts. Global voice and A2UI requests now receive the latest 12 turns plus bounded semantic, procedural, and episodic retrieval; exchanges persist after inference. PostgreSQL adds pgvector/FTS memory projections, Kafka adds candidates/events/current/invalidations plus a real memory worker, nodes use SQLite WAL with a bbolt append-only journal, Python exposes a token-authenticated pinned MiniLM ONNX embedding endpoint, and Cutaway shows actual memory path state. The optional private `memory-lab` profile contains MinIO, Dagster assets, and MLflow. This entry describes source implementation pending live deployment verification and must not yet be described as live.
- AI memory boundary, last verified 2026-09-03: global voice/A2UI turns are independent OpenAI Responses requests with `store:false`; each receives the current utterance plus a fresh bounded fleet/workspace snapshot, selected IDs, active mission ID, and visible plan options, but no prior global turns or scene history. Scene history is an in-memory UI/audit projection capped at 50 and is not model context. Mission Canvas is the only conversational path with continuity: its PostgreSQL-backed mission record supplies the latest 12 mission messages to strategy generation. Target selection and semantic command interpretation still run before that conversation is attached, and no synchronized cross-node conversational memory exists yet.
...
- Mission Planner layout, last verified 2026-09-03: Mission Planner first opens docked right at 350 px, Fleet/Groups first opens docked left at 245 px, and attached windows retain user-resizable height. At 350×245 the Planner becomes a chat-only surface; the redundant Assigned Assets editor is gone because Fleet selection is authoritative. Objective and Map Authoring use progressive disclosure, authoring actions are compact icon buttons with accessible hover labels, and maximized mode remains a balanced two-pane layout.
- Conversational mission approval, last verified 2026-09-03: AI planning returns exactly three bounded choices labeled A/B/C beneath Mission Planner chat and explicitly asks the operator to choose one. A card click requests one exact-hash confirmation; a chat or voice choice such as `go with option A` counts as that confirmation and automatically performs preview, exact-plan authorization, and mission start. The server supplies only current immutable plan IDs/hashes to the assistant and revalidates every lease, state version, policy result, and idempotency boundary. GPT-5.6 Luna is attempted directly for 12 seconds before the bounded Python-service fallback.
...
- The main UI is a map-first floating workspace. Mission creation is initiated from the mission `+` control or selected Fleet/Groups assets; the removed global bottom command dock is replaced by a persistent per-mission Markdown conversation and compact/expanded strategy workspace.
