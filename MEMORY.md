...
- Current task: AI-assisted target selection for empty mission drafts is implemented, verified, and deployed.
- Last meaningful change: an empty AI-assisted mission now sends a bounded availability-aware fleet/group projection to the Python AI service before compilation. GPT-5.6 Luna selects exact vessel IDs and explains its choice; core then validates availability/conflicts, persists the roster, derives geometry, and runs the existing deterministic planning/policy/approval path. Manual planning still requires explicit targets, and provider loss uses a clearly labeled deterministic resolver.
