...
- Current task: the global hold-to-talk workspace assistant is implemented, verified, and deployed on VM 214 and all twelve vessel nodes.
- Last meaningful change: a bottom-right microphone orb now routes VM-local transcription to GPT-5.6 Luna before deciding whether to answer privately by Jarvis/Barbossa, apply bounded semantic workspace actions, or open and populate Mission Planner for mission work. The orb exposes real listening/transcription/inference/TTS/speaking progress without displaying a conversational transcript outside Mission Planner.
...
- Result: VM 214 and all twelve nodes run the global voice UI and bounded agent endpoint. The node binary SHA-256 is `a5c9c898366f6f7db49769ab9afc3ccdc1e49c85c24c3c4b698733765eb76408`. A deployment defect that made the OpenAI runtime copy unreadable to the non-root Go UID was fixed with separate Go/Python UID-owned `0400` volumes. No GitHub-hosted workflow or Proxmox snapshot ran.
