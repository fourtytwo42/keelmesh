...
- Current task: session-scoped chat clearing is complete.
- Last meaningful change: the Assistant chat header now has a confirmed clear action that removes only the current operator/browser transcript from central and node-local exact-turn stores while preserving vector/semantic memories, preferences, scenes, entities, and evidence.
