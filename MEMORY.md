...
- Current task: the clipped Cutaway window is fixed and deployed to VM 214 and all twelve vessel nodes without GitHub-hosted workflows or snapshots.
- Last meaningful change: Cutaway now opens at the largest safe workspace height, compacts its dense pipeline at short desktop heights, keeps its footer and all evidence sections visible at the release desktop viewports, and uses a thin themed internal-scroll fallback for narrower stacked layouts.
