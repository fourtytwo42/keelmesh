...
- Result: every tier reached the expected exact-plan or executing state; Cell A and Cell B each produced six valid acknowledgements for the cross-cell mission where four were required; post-reload chat correctly retained FV Sea Robin / NPC-4108; all test missions and the temporary Beacon Pursuit group were deleted, leaving zero missions and twelve unassigned vessels. A final reload regression fixed audit-history scene restoration so only the newest ordinary scene reopens while pinned, pending-approval, and critical scenes retain their lifecycle. Commits `18df613` and `9663fcc` are pushed to `main`; VM 214 runs `keelmesh/core:main-9663fcc`, and all twelve nodes are healthy on binary SHA-256 `2759ed6193bdd3b09287a251c82dc4fd18d466a27e1d83c251dfc5418552d522`. Both six-node cells converged to one index and one state hash after sequential restart. No voice test, hosted workflow, Proxmox migration, or snapshot ran.
- Follow-up: preserve this AI-chat acceptance path in future mission and agent changes.

### 2026-09-04 - M12 authoritative rollout and repository cleanup complete

- Context: the original migration order would temporarily pressure mini43, and the first Cell A promotion exposed a term-insensitive epoch idempotency collision.
- Decision: preserve the required 2/2/2 voter distribution but move VM 233 off mini43 before its two inbound voters; barrier before reading epoch state and bind epoch-advance identity to the Raft term. Keep simulated and shadow modes as explicit rollback paths.
- Files: `internal/coordination/manager.go`, `internal/coordination/manager_test.go`, `internal/domain/nodefleet.go`, `internal/domain/nodefleet_test.go`, `scripts/verify_m2.py`, `infrastructure/m12-target-topology.json`, `.gitignore`, and M12 status/documentation.
- Commands/tests: full Go tests/vet and production web build; local twelve-process consensus/TLS suite; live Cell A and B leader replacement; Cell A 4/2 commit and 3/3 write rejection using radio-only faults; follower convergence; six-signature single-cell and cross-cell proofs; complete M1-M7/M11/M12 verification twice; final live `verify_m12.py` after core recreation.
- Result: VM 214 runs `keelmesh/core:main-3979407` with `KEELMESH_COORDINATION_MODE=raft`. Cell A is healthy at term 12/epoch 10 and Cell B at term 11/epoch 9. All twelve nodes run binary SHA-256 `fa78e6c55656931ca368255673df0ef95860bb7f966fa5447a11aa1733fa9fd0`. No radio fault, hosted workflow, or Proxmox snapshot remains/runs. Root build-transfer artifacts, QA PNGs, bundles, generated test output, temporary VM 214 worktrees/binaries, and duplicate node `m12.conf` drop-ins were removed; product map/sprite/docs assets and runtime data were retained.
- Follow-up: treat thin-pool overcommit/disabled auto-extension as infrastructure debt; do not change storage or create snapshots without a separate preflight and explicit authorization.
...
### 2026-09-04 - M12 compatibility stabilization and migration gate

- Context: two consecutive live M1-M12 passes exposed background health/topology observations racing optimistic authority versions; M6 also compared asynchronous simulation positions around a preview.
- Decision: keep probe/topology updates observational and outside Arena/Fleet authority versions, matching the M12 rule that raw telemetry is not replicated. Verify previews through unchanged mission version/status plus `nothing_sent` and exact plan hash.
- Files: `internal/arena/manager.go`, `internal/fleetops/manager.go`, and `scripts/verify_m6.py`.
- Commands/tests: full Go tests and vet plus focused Fleet/Arena/API tests; a fresh twelve-process two-cell test; live M6/M7/M12 twice; all twelve guests inspected for memory, service RSS, and disk use.
- Result: M6, M7, and M12 pass twice consecutively on VM 214. Vessel guests use about 343-376 MiB total available working memory and the Raft service uses about 8-16 MiB RSS, but each VM remains allocated 2 GiB. Target VMs 222, 223, 224, 229, and 233 have no Proxmox snapshots. mini43 thin data is 64.04% used (545.83/852.33 GB), but its RAM headroom and unresolved thin metadata block migration. VM 214 is healthy on `keelmesh/core:m12-shadow-4`. No hosted workflow, migration, or snapshot ran.
...
- M7 is live as an Arena vertical slice plus a physical node fabric. VM 214 exposes neutral/referee state and signed-leader-aware Player B ingress; twelve Ubuntu vessel VMs run the same Go/UI node binary with faction-pinned APIs across separate management (`192.168.50.0/24`) and simulated-radio (`10.77.0.0/24`) interfaces. M12 now supplies authoritative per-faction Raft replication and node mTLS; simulated Arena world effects remain centrally refereed. Twelve independent Python/TTS/STT runtimes remain follow-ups.
...
- M11 node memory, last verified 2026-09-04: all twelve vessel VMs run binary SHA-256 `fa78e6c55656931ca368255673df0ef95860bb7f966fa5447a11aa1733fa9fd0` and report `node-local`. Each has a mode-0700 memory directory, SQLite WAL store, and mode-0600 bbolt journal. A real A-01 test retained one explicit preference, two turns, and one trusted A2UI scene across service restart. Signed `MemoryBundleV1` contracts exist, but real radio-plane bundle exchange remains follow-up work and must not be claimed live.
...
- Result: single-vessel, group, dynamic-contact, looping patrol, and cross-cell missions behaved as requested. Explicit constraints survived planning, follow routes remained dynamic, management commands did not authorize plans, and short-term context survived reload. Test missions and group were removed afterward. The final browser reload restored three legitimate critical scenes instead of fifteen historical ordinary scenes. Final node SHA-256 is `2759ed6193bdd3b09287a251c82dc4fd18d466a27e1d83c251dfc5418552d522`; both cells reconverged after deployment.
