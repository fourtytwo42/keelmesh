...
- Current task: the mission-roster, default docking, automatic voice, compact authoring, environment/footer, and loop-behavior pass is implemented, verified, and deployed.
- Last meaningful change: Fleet/Groups first opens docked left at 245 px and Mission Planner first opens docked right at 350 px; only those two windows expose docking. Fleet selection is the active mission roster, opening a mission restores that selection, and membership/loop changes safely invalidate stale authority for a new plan. Navy replies use Jarvis automatically and Pirate replies use Captain Barbossa automatically. Mission authoring is icon-dense, the environment strip is centered below mission tabs, and non-loop missions hold at the final marker while loop missions start a fresh signed revision back toward the first route marker.
...
- Mission creation, last verified 2026-09-03: pressing `+` never requires a prior Fleet selection. With no selection it creates a uniquely named, zero-asset persistent draft and opens Mission Planner; with a selection it carries those assets into the draft. Inside Planner, an operator can assign one complete operational group, apply any current mixed Fleet selection, clear the assignment, or open Fleet/Groups. Route generation and authority remain disabled until targets and required geometry exist.
