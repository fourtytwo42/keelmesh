...
- Current task: visually and physically coherent idle formation holding is implemented, verified, deployed, and pushed in commit `cfa1faf`.
- Last meaningful change: operational groups now persist a compass formation heading, rotate every station slot around it, and align all idle hull headings instead of retaining each vessel's last inward bearing to its slot. Group details expose heading independently from spacing and formation shape.
